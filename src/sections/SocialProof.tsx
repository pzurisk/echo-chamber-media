...
function StatItem({
  ...
  suffix,
  inView,
}: (typeof stats)[0] & { inView: boolean }) {
  const count = useCountUp(target, inView);

  return (
    <div className="text-center">
      <span className="font-heading text-4xl md:text-5xl text-brand-gold">
        {count}
        {suffix}
      </span>
      <p className="mt-2 text-sm uppercase tracking-editorial text-brand-gray font-body">
        {label}
      </p>
    </div>
  );
...
export default function SocialProof() {
  ...
  return (
    <section
      ref={ref}
      className="relative w-full py-20 md:py-28 bg-brand-black border-t border-b border-brand-charcoal"
    >
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-12 mb-16">
          {awards.map((award) => (
            <div key={award} className="flex items-center gap-3">
              ...
              <span className="text-sm md:text-base text-brand-off-white font-body tracking-wide">
                {award}
              </span>
            </div>
          ))}
